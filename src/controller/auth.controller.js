...
class AuthController {
    async signUp(req, res, next) {
        try {
            ...
            if( !String(password).match(constants.passwordRegex) || !String(email).match(constants.emailRegex)) {
               return res.status(400).json({msg: 'Email or password is not right format'})
            }
            if(!email || !password || !username) return res.status(400).json({msg: 'Các trường không được để trống!'})
            const emailExisted = await Userm.findOne({email: email});

            if(emailExisted) return res.status(409).json({msg:'Tài khoản đã tồn tại!!!'})
            const user = new Userm({
                email, password, firstName: username
            })
            const usersaved = await user.save();

            return res.status(200).json({msg : 'Bạn đã đăng ký thành công!', data: {...usersaved._doc, password: ''}});
        } catch (error) {
    ...
    async refreshToken(req, res) {
        try {
            ...
            if (!rf_token) return res.status(400).json({ msg: "Hãy đăng nhập lại lần nữa !" });
            jwt.verify(
              rf_token,
    ...
    async forgotPass(req, res, next) {
        try {
            ...
            if(!user) return res.status(400).json({msg: 'Email is not existed!!!', status:'fail'});
            const token = await jwt.sign({_id: user._doc._id}, process.env.ACCESS_TOKEN_SECRET, {expiresIn: '1h'})
            ...
            res.status(200).json({msg: 'Send the link to your email successfully!'});
        } catch (error) {
    ...
    async changePassword (req, res, next) {
        try {
         ...
         if(!password || !String(password).match(constants.passwordRegex)) return res.status(400).json({msg: `Password's format is not correct!!!`})
         ...
         return res.status(200).json({msg: 'Your password was changed successfully!', isOk: true});
        } catch (error) {
